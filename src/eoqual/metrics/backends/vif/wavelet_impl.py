...
# Necessite: pip install pyrtools
#
# STATUT LICENCE (voir THIRD_PARTY_LICENSES.md, racine du dépôt) : MIT
# (vérifié) — compatible avec la licence MIT d'eoqual-metrics.
import numpy as np
...
def vif(img_ref, img_dist, wavelet='steerable', full=False):
    ...
    if wavelet == 'steerable':
        ...
        pyr_ref = SPyr(img_ref, 4, 5, 'reflect1').pyr_coeffs # type: ignore
        pyr_dist = SPyr(img_dist, 4, 5, 'reflect1').pyr_coeffs # type: ignore
        subband_keys = []
    ...
    if not full:
        return np.mean(nums + 1e-4) / np.mean(dens + 1e-4) # type: ignore
    else:
        return np.mean(nums + 1e-4) / np.mean(dens + 1e-4), (nums + 1e-4), (dens + 1e-4) # type: ignore
...
def vif_spatial(img_ref, img_dist, k=11, sigma_nsq=0.1, stride=1, full=False):
    ...
    vif_val = np.sum(np.log(1 + g**2 * var_x / (sv_sq + sigma_nsq)) + 1e-4) / np.sum(np.log(1 + var_x / sigma_nsq) + 1e-4) # type: ignore
    if (full):
...
def msvif_spatial(img_ref, img_dist, k=11, sigma_nsq=0.1, stride=1, full=False):
    ...
    msvifval = np.sum(nums) / np.sum(dens) # type: ignore
