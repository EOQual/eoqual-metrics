...
#        print("%f %f %f" % (k, ((3-4*k)/(4*c1)), c2**2 + 4*c3*math.log((3-4*k)/(4*c1)) ))
        d = c2**2 + 4*c3*math.log((3-4*k)/(4*c1))
        if d < 0: d=0
        return (1/(2*c3)) * (c2 - math.sqrt(d))
    else:
        print(f"warning: GGRF inverse of {k} is not defined")
        return numpy.nan
...
def estimate_aggd_params(x):
    ...
    logger.debug(f"stddev_left={stddev_left}  stddev_right={stddev_right}  r_hat={r_hat}  y_hat={y_hat}")
...
def niqe(img):
    ...
    # load model
    fileModel = os.path.join(os.path.dirname(__file__), 'models/modelparameters.mat')
    if os.path.isfile(fileModel):
        model_mat = scipy.io.loadmat(fileModel)
    else:
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), fileModel)
    ...
    features_mu = numpy.mean(features, axis=0) # type: ignore
    features_cov = numpy.cov(features.T) # type: ignore
