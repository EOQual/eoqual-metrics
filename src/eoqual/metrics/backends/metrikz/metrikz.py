...
# -*- coding: utf-8 -*-
"""
Template.py : Compare two or more images using the following metrics:
    - MSE
    - PSNR
    - SNR
    - SSIM
...
https://bitbucket.org/kuraiev/pymetrikz/overview

STATUT LICENCE (voir THIRD_PARTY_LICENSES.md, racine du dépôt) :
Ce fichier est vendored depuis pymetrikz. Plusieurs fonctions ci-dessous
portent leur propre en-tête `__license__ = "GPL"` / `"GPLv2"` (voir plus
bas) — ce fichier N'EST PAS sous la licence MIT du reste d'eoqual-metrics.
Utilisé comme SEUL algorithme de `wsnr` (🔴 bloquant) et comme alternative
optionnelle pour `mse`, `rmse`, `snr`, `psnr`, `psnrc`, `ssim`, `uqi`, `vif`.
"""
...
def ssim(reference, query, k=(0.01, 0.03), dynamic_range=255.0, MAP=False):
    ...
    reference     : original image data.
    query         : modified image data to be compared.
    dynamic_range : width of color map
    map           : output map
    ...
    """
    def __get_kernels():
        kern1, kern2 = map(lambda x: (x * dynamic_range) ** 2, (k[0], k[1]))
        return kern1, kern2
