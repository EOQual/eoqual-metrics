...
try:
	...
	if sys.platform == 'win32':
		libsvm = CDLL(path.join(dirname, r'../lib/libsvm.dll'))
	elif sys.platform == 'darwin':
		libsvm = CDLL(path.join(dirname, r'../lib/libsvm.dylib'))
	elif sys.platform == 'linux':
		libsvm = CDLL(path.join(dirname, r'../lib/libsvm.so.2'))
	else:
		raise Exception(f"Plateforme [{sys.platform}] non traitée pour libsvm")
except:
