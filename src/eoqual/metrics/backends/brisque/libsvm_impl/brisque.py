...
def AGGDfit(structdis):
    ...
    vectfunc = np.vectorize(func, otypes=[np.float64], cache=False)  # type: ignore
...
def compute_features(img):
    ...
    for itr_scale in range(scalenum):
        ...
        for itr_shift in range(1, len(shifts) + 1):
            ...
            M = np.float32([[1, 0, reqshift[1]], [0, 1, reqshift[0]]])  # type: ignore
