...
def piqe(im):
    ...
    im = np.pad(im, ((0, rowsPad), (0, columnsPad)), 'edge') # type: ignore
    ...
    for i in np.arange(0, imnorm.shape[0]-1, blockSize):
        for j in np.arange(0, imnorm.shape[1]-1, blockSize):
            ...
            if(blockVar > activityThreshold):
                ...
                if WNDC*pow(1-blockVar, 2) + WNC*pow(blockVar, 2) > 0: # type: ignore
                    BlockScores.append(
                        WNDC*pow(1-blockVar, 2) + WNC*pow(blockVar, 2)) # type: ignore
