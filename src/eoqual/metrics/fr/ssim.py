...
# piq (Apache-2.0) — remplace IQA_pytorch pour ssim/msssim, voir
# backends/piq/common.py et THIRD_PARTY_LICENSES.md.
try:
...
def ssim(
    ...
    **kwargs,
    ...
    algo : str, optional
        ...
        * ``"piq"`` — Apache-2.0, GRAY et RGB, voir ``backends/piq/common.py``
        * ``"IQA_pytorch"`` — legacy
    **kwargs
    ...
    if algo == "piq":
        ...
        return float(piq.ssim(dist, ref, data_range=1.0).item())

    if algo == "IQA_pytorch":
        if not _IQA or not _TORCH:
            raise ImportError(
                "IQA_pytorch et torch sont requis pour algo='IQA_pytorch' : "
                "pip install \"eoqual-metrics[deep]\"."
            )
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ref = prepare_image(GT).to(device)
        dist = prepare_image(P).to(device)
    ...
    **kwargs,
    ...
    algo : str, optional
        ...
        * ``"piq"`` — Apache-2.0, voir ``backends/piq/common.py`` ;
          nécessite des images d'au moins 161×161 pixels (5 échelles)
        * ``"IQA_pytorch"`` — legacy
    **kwargs
    ...
    if algo == "piq":
        ...
        ref = prepare_image(GT).to(device)
        dist = prepare_image(P).to(device)
        return float(piq.multi_scale_ssim(dist, ref, data_range=1.0).item())

    if algo == "IQA_pytorch":
        if not _IQA or not _TORCH:
            logger.error("IQA_pytorch et torch sont requis pour algo='IQA_pytorch'.")
            raise ImportError(
                "IQA_pytorch et torch sont requis : pip install \"eoqual-metrics[deep]\"."
            )
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ref = prepare_image(GT).to(device)
        dist = prepare_image(P).to(device)
