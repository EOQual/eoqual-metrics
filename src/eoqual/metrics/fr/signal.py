...
def wsnr(
    ...
    """
    Calcule le Weighted Signal-to-Noise Ratio (WSNR), pondéré par une
    fonction de sensibilité au contraste (CSF, modèle de Mannos & Sakrison).
    ...
    algo : str, optional
        ...
        * ``"numpy"`` (défaut)
        * ``"metrikz"`` — legacy, conservé pour compatibilité ascendante
          (voir ``THIRD_PARTY_LICENSES.md`` : vendored GPLv2)

    Returns
    -------
    float
        Valeur du WSNR (dB). Une valeur plus élevée indique une meilleure
        qualité. ``inf`` si les deux images sont identiques.
    ...
    GT, P = initial_check(GT, P)
    if algo == "numpy":
        from ..backends.wsnr.numpy_impl import wsnr as _wsnr_numpy
        return _wsnr_numpy(GT, P)
    if algo == "metrikz":
