...
1. Détection des contours (Sobel) : les zones de contour sont les
   candidates pour évaluer la netteté.
2. Analyse des gradients autour des contours : on calcule la direction
   (angle) du gradient en chaque point, et on cherche spécifiquement les
   bords horizontaux/verticaux dominants (les plus perceptuellement
   importants).
3. Mesure des largeurs de transitions : pour un bord, on regarde combien
   de pixels il faut pour passer d'un minimum local à un maximum local
   (transition sombre vers clair ou l'inverse) — plus cette transition
   est courte, plus le bord est net. Une correction (JNB, Just Noticeable
   Blur) retire les transitions trop fines pour être perceptibles.
4. Agrégation en blocs : l'image est découpée en blocs (par défaut
   32×32) ; pour chaque bloc, on calcule la largeur moyenne des
   transitions. Seuls les blocs les plus nets (par ex. les 22%
   supérieurs) sont pris en compte, car la netteté perceptuelle dépend
   surtout des zones les plus contrastées.
5. Score final : défini comme le nombre de bords nets retenus divisé par
   la somme de leurs largeurs — plus le score est grand, plus l'image est
   perçue comme nette.
