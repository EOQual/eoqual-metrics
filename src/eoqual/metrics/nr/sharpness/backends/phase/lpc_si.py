...
1. Décomposition multi-échelle : l'image est transformée (typiquement par
   complex wavelet transform) ; chaque pixel est représenté par amplitude
   et phase à plusieurs échelles.
2. Analyse de la cohérence de phase : en bord net, les composantes
   d'échelles différentes s'alignent en phase ; en bord flou, cette
   cohérence se dégrade.
3. Mesure de la cohérence locale : pour chaque position, on calcule un
   indice de phase congruency (ou phase coherence) — essentiellement une
   mesure de stabilité des phases entre échelles.
4. Agrégation globale : on combine ces indices locaux en un score de
   netteté global — un score élevé signifie une image jugée nette.
