"""
Script exploratoire manuel — PAS un test automatisé, PAS un exemple à
suivre. Utilise directement l'API IQA_pytorch (legacy, voir
THIRD_PARTY_LICENSES.md et backends/piq/common.py pour le remplacement
`piq` retenu) plutôt que le registre eoqual-metrics — conservé pour
mémoire (historique de l'exploration IQA_pytorch avant l'intégration au
registre). Pour un usage réel, voir plutôt `examples/`.

Exécution : `cd tests/exploratory && python recover_iqa_pytorch.py`
