"""
Script exploratoire manuel — PAS un test automatisé (pas de pytest, pas
exécuté en CI, pas lancé par run_tests.sh).

STATUT — piste écartée, conservée pour mémoire seulement
--------------------------------------------------------------------
Ce script évaluait `pyiqa` (chaofengc/IQA-PyTorch) comme remplacement de
IQA_pytorch (non maintenu, cassé avec torch récent). **`pyiqa` s'est révélé
sous licence PolyForm-Noncommercial-1.0.0** — pas open-source au sens OSI,
usage commercial explicitement interdit — donc écarté. Le remplacement
effectivement intégré au registre est **`piq`** (Apache-2.0, voir
`backends/piq/common.py`, `fr/perceptual.py`, `fr/ssim.py`), qui couvre
dists/fsim/gmsd/vif/ssim/msssim/vsi/lpips_vgg. `nlpd`/`mad` n'ont pas
d'équivalent dans `piq` et restent sur IQA_pytorch (legacy) — voir
`THIRD_PARTY_LICENSES.md`. Ce fichier n'est plus une piste active ; il
documente pourquoi `pyiqa` n'a pas été retenu.

Install (pour rejouer l'exploration, non recommandé)
-------
...
from eoqual.metrics.core.image_utils import prepare_image
